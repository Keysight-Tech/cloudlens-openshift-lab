...
if ! oc whoami &>/dev/null 2>&1; then
    ...
    # 6a: Grant privileged SCC to cloudlens-sensor ServiceAccount
    # Required for the CloudLens sensor sidecar (host volumes + NET_ADMIN/SYS_MODULE)
    log_info "Granting privileged SCC to cloudlens-sensor ServiceAccount..."
    oc create serviceaccount cloudlens-sensor -n default --dry-run=client -o yaml | oc apply -f - 2>/dev/null
    oc adm policy add-scc-to-user privileged -z cloudlens-sensor -n default 2>/dev/null || true
    log_success "cloudlens-sensor ServiceAccount configured with privileged SCC"

    # 6b: Inject the nginx HTML ConfigMap with OpenShift-branded landing page
    # (Same approach as EKS lab - apply ConfigMap first, Deployment references it)
    log_info "Deploying nginx HTML landing page (OpenShift branded)..."
    oc apply -f - <<'YAMLEOF' 2>&1
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  index.html: |
    ...
    <head>
        ...
        <title>Keysight OpenShift CloudLens Tap</title>
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .particle { position: absolute; width: 4px; height: 4px; background: rgba(255,255,255,0.7); border-radius: 50%; animation: float 6s ease-in-out infinite; }
            @keyframes float { 0%,100% { transform: translateY(0px) rotate(0deg); } 50% { transform: translateY(-20px) rotate(180deg); } }
            .container { text-align: center; z-index: 10; background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 20px; padding: 60px 40px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); border: 1px solid rgba(255,255,255,0.2); animation: fadeInUp 1s ease-out; }
            @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
            h1 { font-size: 3.5rem; font-weight: 700; color: #ffffff; margin-bottom: 20px; text-shadow: 2px 2px 4px rgba(0,0,0,0.3); animation: glow 2s ease-in-out infinite alternate; }
            ...
            .subtitle { font-size: 1.2rem; color: #e0e6ff; margin-bottom: 30px; opacity: 0.9; }
            .status-indicator { display: inline-flex; align-items: center; gap: 10px; background: rgba(76,175,80,0.2); padding: 10px 20px; border-radius: 25px; border: 1px solid rgba(76,175,80,0.4); margin-top: 20px; }
            .status-dot { width: 12px; height: 12px; background: #4CAF50; border-radius: 50%; animation: pulse 1.5s infinite; }
            ...
            .status-text { color: #ffffff; font-weight: 500; }
            .protocol-info { position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.1); padding: 10px 15px; border-radius: 10px; color: #ffffff; font-size: 0.9rem; backdrop-filter: blur(5px); }
            .keysight-logo { font-size: 1.8rem; color: #ff6b35; font-weight: bold; margin-bottom: 10px; }
            .platform-badge { display: inline-block; background: rgba(255,107,53,0.2); padding: 8px 16px; border-radius: 15px; color: #ff6b35; font-weight: 600; margin-top: 15px; border: 1px solid rgba(255,107,53,0.4); }
            .openshift-badge { display: inline-block; background: rgba(238,0,0,0.15); padding: 6px 14px; border-radius: 15px; color: #ff6b6b; font-weight: 600; margin-top: 10px; margin-left: 8px; border: 1px solid rgba(238,0,0,0.3); font-size: 0.9rem; }
            @media (max-width: 768px) { h1 { font-size: 2.5rem; } .container { padding: 40px 20px; margin: 20px; } .protocol-info { position: static; margin-top: 20px; } }
        </style>
    </head>
    <body>
        ...
        <div class="protocol-info"><span id="protocolText">HTTPS (Route)</span></div>
        <div class="container">
            <div class="keysight-logo">&#9889; KEYSIGHT</div>
            <h1>Welcome to Keysight OpenShift CloudLens Tap</h1>
            <p class="subtitle">Advanced OpenShift Container Monitoring &amp; Analytics</p>
            <div class="platform-badge">AWS ROSA HCP Platform</div>
            <span class="openshift-badge">&#128994; OpenShift Route</span>
            <div class="status-indicator">
                <div class="status-dot"></div>
                <span class="status-text">Service Active &amp; Monitored</span>
            </div>
        </div>
        <script>
            function updateProtocolInfo() {
                const el = document.getElementById('protocolText');
                if (window.location.protocol === 'https:') { el.textContent = 'HTTPS (Route TLS Edge)'; el.style.color = '#4CAF50'; }
                else { el.textContent = 'HTTP (Standard)'; el.style.color = '#FFA726'; }
            }
            updateProtocolInfo();
            function createParticle() {
                const p = document.createElement('div'); p.className = 'particle';
                p.style.left = Math.random() * 100 + '%'; p.style.top = Math.random() * 100 + '%';
                p.style.animationDelay = Math.random() * 6 + 's'; document.body.appendChild(p);
...
YAMLEOF
    log_success "nginx-html-config ConfigMap created"

    # 6c: Deploy nginx-demo (bitnami/nginx + CloudLens sensor) with CLMS IP substituted
    log_info "Deploying nginx-demo with CloudLens sensor sidecar..."
    if [[ -n "$CLMS_PRIVATE_IP" ]]; then
        ...
        log_success "nginx-demo deployed (CLMS: $CLMS_PRIVATE_IP, prefix: $DEPLOYMENT_PREFIX)"
    else
        log_warn "CLMS private IP not detected — applying without substitution."
        log_warn "Edit $NGINX_MANIFEST with correct IPs, then: oc apply -f $NGINX_MANIFEST"
        oc apply -f "$NGINX_MANIFEST" 2>&1 || true
    fi

    # 6d: Wait for nginx-demo pods
    log_info "Waiting for nginx-demo pods..."
    oc rollout status deployment/nginx-demo -n default --timeout=180s 2>&1 || \
        log_warn "nginx-demo not ready yet. Check: oc get pods -n default"
    log_success "nginx-demo deployed (2 replicas)"

    # 6e: Show OpenShift Route URL
    log_info "Getting OpenShift Route URL..."
    ...
    [[ -z "$NGINX_ROUTE" ]] && log_warn "Route not ready. Check: oc get route nginx-demo -n default"
fi
